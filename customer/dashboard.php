...
if ($storage::class === 'App\Storage\FileStorage') {
   $customers = $storage->loadData("users.json");
    ...
    foreach ($transactions as $transaction) {
      if ($transaction['account'] == $customer_email|| $transaction['toAccount'] == $customer_email) {
            ...
            <div class="px-4 sm:px-6 lg:px-8">
              ...
              <?php if ($hasTransactions) :?>
                    <table class="min-w-full divide-y divide-gray-300">
                      ...
                      <tbody class="divide-y divide-gray-200 bg-white">
                        <?php foreach($transactions as $transaction) : 
                          if ( $transaction["toAccount"] == $_SESSION['email'] && $transaction["type"] === "transfer"  ):
                        ?>
                        <tr>
                          <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-800 sm:pl-0">
                          ...
                          <td
                            class="whitespace-nowrap px-2 py-4 text-sm font-medium <?= $transaction['type'] == "transfer" ? "text-emerald-600" : "text-red-600"  ?> ">
              ...
              <?php else : ?>
                <div class="p-4 text-center text-gray-500">
                  <p>No transactions found for <?= $customer_email; ?>.</p>
                </div>
              <?php endif; ?>
                  </div>
                </div>
